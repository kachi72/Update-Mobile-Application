    ...
    private void initWorker() {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                articles = db.articleDao().getArticlesByCategory("AI/ML");
                if (articles.isEmpty()){
                    new Handler(Looper.getMainLooper()).post(new Runnable() {
                        @Override
                        public void run() {
                            hideLoadingIcon();
                            Toast.makeText(OfflineAiActivity.this, "There are no saved posts in the database for this category", Toast.LENGTH_LONG).show();
                            showNoArticleText();
                        }
                    });
                }
